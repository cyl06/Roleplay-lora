"""Judge base model generations, save to basic/results/."""
import json
import os
import re
import torch
...
def main():
    ...
    gen_files = sorted(RESULT_DIR.glob("generations_*.jsonl"))
    if not gen_files:
        print("No generation files found in basic/results/")
        return
    ...
    for gen_file in gen_files:
        ...
        per_judge_path = RESULT_DIR / f"judgements_{model_key}.jsonl"

        if per_judge_path.exists():
            print(f"  SKIP {per_judge_path.name}: already exists")
            continue

        generations = load_jsonl(str(gen_file))
        if not generations:
            continue
        ...
        dump_jsonl(judged, str(per_judge_path))
        print(f"  Saved {len(judged)} → {per_judge_path.name}")

    # Merge
    all_judged = []
    for jf in sorted(RESULT_DIR.glob("judgements_*.jsonl")):
        all_judged.extend(load_jsonl(str(jf)))
    if all_judged:
        dump_jsonl(all_judged, str(RESULT_DIR / "judgements.jsonl"))
        print(f"\nTotal judged: {len(all_judged)} → basic/results/judgements.jsonl")
